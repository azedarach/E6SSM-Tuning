...
#ifndef lowE6SSM_UTILITIES_H
#define lowE6SSM_UTILITIES_H
...
class lowE6SSM_parameter_getter {
public:
   Eigen::ArrayXd get_parameters(const lowE6SSM<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const lowE6SSM<Two_scale>&) const {
      using namespace lowE6SSM_info;
      return std::vector<std::string>(parameter_names,
                                      parameter_names + NUMBER_OF_PARAMETERS);
   }
};

class lowE6SSM_spectrum_plotter {
public:
   lowE6SSM_spectrum_plotter();
   ~lowE6SSM_spectrum_plotter() {}

   template <class T>
   void extract_spectrum(const lowE6SSM<T>&);
...
template <class T>
void lowE6SSM_spectrum_plotter::extract_spectrum(const lowE6SSM<T>& model)
{
   spectrum.clear();
...
template <class Scalar, int M, int N>
std::valarray<double> lowE6SSM_spectrum_plotter::to_valarray(const Eigen::Array<Scalar, M, N>& v)
{
