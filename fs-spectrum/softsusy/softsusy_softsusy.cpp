...
/// DH:: Simplified version including only the stop and sbottom contributions.
/// From hep-ph/9606211's appendix. It should be done at MSusy to minimize the
/// 1-loop contributions. Only call if you've calculated drbarpars.
/// inputs are running top/bottom masses: call at MSusy only
double MssmSoftsusy::doCalcTadpole1oneLoopSimple(double mt, double sinthDRbar) {
...
/// DH:: Simplified version including only stop and sbottom contributions
void MssmSoftsusy::calcTadpole1Ms1loopSimple(double mt, double sinthDRbar) { 
  
  t1OV1Ms1loop = doCalcTadpole1oneLoopSimple(mt, sinthDRbar);
...
/// 1-loop contributions. Only call if you've calculated physpars
/// inputs are running top/bottom masses. Call at MSusy
double MssmSoftsusy::doCalcTadpole2oneLoopSimple(double mt, double sinthDRbar) {
/// CHECKED
 if (forLoops.mu(1, 3) == 0.0 || forLoops.mu(2, 3) == 0.0) {
   if (PRINTOUT > 1)
    cout << "Trying to calculate tadpole without having first calculated"
...
/// DH:: Simplified version including only stop and sbottom contributions
void MssmSoftsusy::calcTadpole2Ms1loopSimple(double mt, double sinthDRbar) {/// CHECKED
  t2OV2Ms1loop = doCalcTadpole2oneLoopSimple(mt, sinthDRbar); 
  if (testNan(t2OV2Ms1loop)) {
    flagNoMuConvergence(true);
    t2OV2Ms1loop = 0.0;
  }
...
bool MssmSoftsusy::higgs(int accuracy, double piwwtMS, double pizztMS) {
  ...
  // DH:: note print out
  //cout << "full temp(1) = " << temp(1) << endl;
  //cout << "full temp(2) = " << temp(2) << endl;
  ...
  if (accuracy > 0) {
	...
	t2OV2Ms1loop + dMA;
      ...
      // DH:: note print out
      //cout << "full poleMasq = " << poleMasq << endl;
...
/// DH:: Simplified version keeping only the effects of stops and sbottoms.
/// Make sure DR bar parameters have been calculated first using stops() and
/// sbottoms(). Likewise the tadpoles must be recalculated with only
// the stop and sbottom contributions, using the versions named "...Simple".
// Note calculates only the lightest CP-even Higgs mass.
bool MssmSoftsusy::higgsSimple(int accuracy, double piwwtMS, double pizztMS) {
  ...
  double sinb2 = sqr(sinb), cosb2 = sqr(cosb), mzPole = displayMz(), 
    mzRun2 = sqr(displayMzRun());
  //double mApole = physpars.mA0; /// physical value
  double mApole = forLoops.mA0; //< DH:: changed physpars to forLoops
  ///  double mApole2 = sqr(mApole);
  ...
  if (accuracy > 0) {
    /// one-loop radiative corrections included in sigma
    // DH:: NB as written forLoops must be calculated first!!
    p = forLoops.mh0; //< DH:: changed physpars to forLoops, check that this will work!
    sigmaMh(1, 1) = pis1s1Simple(p, q); 
    sigmaMh(1, 2) = pis1s2Simple(p, q); 
    sigmaMh(2, 2) = pis2s2Simple(p, q); 
  ...
  //double piaa = piAA(mApole, displayMu()); 
  double piaa = piAASimple(displayDrBarPars().mA0, displayMu());//< DH:: changed to use DR bar m_A
  // DH:: here I am going to try different expressions in terms of mu and m_3^2
  // instead of m_Hd^1 and m_Hu^2, since mu and m_3^2 are inputs to the tuning calculation,
  // but this may not be correct, so don't be too trusting...
  // double poleMasq = (displayMh2Squared() - displayMh1Squared() )
  //   / cos(2.0 * beta) - sqr(mzPole);
  double poleMasq = displayM3Squared()/(sin(beta)*cos(beta));
  
  // DH:: only use 1-loop tadpoles for our tuning calculation
  if (accuracy > 0) {
      // poleMasq = 
      // 	(displayMh2Squared() - displayTadpole2Ms() - 
      // 	 displayMh1Squared() + displayTadpole1Ms()) / 
    ...
    poleMasq = displayM3Squared()/(sin(beta)*cos(beta))-piaa
    +sqr(sin(beta))*t1OV1Ms1loop +sqr(cos(beta))*t2OV2Ms1loop + dMA;

    //cout << "poleMasq = " << poleMasq << endl;
...
void MssmSoftsusy::physical(int accuracy) {
  ...
  if (higgsTachyon) { flagTachyon(h0); flagTachyon(essmsoftsusy::A0); 
    flagTachyon(hpm); }
...
/// DH:: The part of calcDrBarPars that calculates only the stops
void MssmSoftsusy::stops()
{
  /// We want to set mu to be the one obtained from the tree-level Higgs
  /// potential for these purposes
  double savedMu = displaySusyMu();
  ...
  DoubleVector mstopDRbar(mSq.apply(zeroSqrt));   
  int i; for (i=1; i<=2; i++) {
    eg.mu(i, family) = mstopDRbar(i);    
  }

  //setDrBarPars(eg);
  forLoops.mu(1,3) = eg.mu(1,3);
  forLoops.mu(2,3) = eg.mu(2,3);
  forLoops.thetat = eg.thetat;
...
/// DH:: The part of calcDrBarPars that calculates only the sbottoms
void MssmSoftsusy::sbottoms()
{
  /// We want to set mu to be the one obtained from the tree-level Higgs
  /// potential for these purposes
  double savedMu = displaySusyMu();
  ...
  if (MIXING > 0) {
    DoubleMatrix diagDown(displayYukawaMatrix(YD));

    
    DoubleMatrix diagTriDown(displayTrilinear(DA));
  ...
  int i; for (i=1; i<=2; i++) {
    eg.md(i, family) = msbotDRbar(i);
  }

  //setDrBarPars(eg);
  forLoops.md(1,3) = eg.md(1,3);
  forLoops.md(2,3) = eg.md(2,3);
  forLoops.thetab = eg.thetab;
...
// DH:: modified routine including only stop and sbottom contributions
double MssmSoftsusy::pis1s1Simple(double p, double q) const {

  // Stops and sbottoms need to be calculated beforehand
  drBarPars tree(displayDrBarPars());
...
// DH:: simplified routine including only stop and sbottom contributions.
double MssmSoftsusy::pis1s2Simple(double p, double q) const {
  // Make sure stops and sbottoms are calculated beforehand
  drBarPars tree(displayDrBarPars());
...
// DH:: simplified routine including only stop and sbottom contributions
double MssmSoftsusy::pis2s2Simple(double p, double q) const {
  // Make sure stop and sbottoms are calculated beforehand
  drBarPars tree(displayDrBarPars());
  ...
  return (fermions + sfermions) / (16.0 * sqr(PI));
}
...
// DH:: simplified version using only stop and sbottom contributions
double MssmSoftsusy::piAASimple(double p, double q) const {/// checked 30.07.03
  drBarPars tree(displayDrBarPars());
 ...
 if (tree.mu(1, 3) == 0.0 || tree.mu(2, 3) == 0.0) {
  ...
  double sfermions = stops + sbots;

  // return (fermions + sfermions + higgs + neutralinos + charginos) 
  //   / (16.0 * sqr(PI));

  return (fermions + sfermions)
    / (16.0 * sqr(PI));
}
...
void pMSSMBcs(MssmSoftsusy & m, const DoubleVector & inputParameters) {
  int i;
  for (i=1; i<=3; i++) m.setGauginoMass(i, inputParameters.display(i));
  if (inputParameters.display(25) > 1. && m.displaySetTbAtMX()) 
    m.setTanb(inputParameters.display(25));
