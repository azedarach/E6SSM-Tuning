...
#ifndef SOFTSUSY_SOFTSUSY_H
#define SOFTSUSY_SOFTSUSY_H
...
public:
  ...
  */
  ...
  /// DH:: These are the simplified routines to use in the fine tuning scan.
  /// They calculate the relevant quantities including only the stop and sbottom
  /// contributions to them, neglecting everything else.
  virtual void stops();
  virtual void sbottoms();
  /// Calculates transverse part of Higgs self-energy: for p=external momentum,
  /// Q=renormalisation scale
  double pis1s1Simple(double p, double q) const;
  /// Calculates transverse part of Higgs self-energy: for p=external momentum,
  /// Q=renormalisation scale
  double pis1s2Simple(double p, double q) const;
  /// Calculates transverse part of Higgs self-energy: for p=external momentum,
  /// Q=renormalisation scale
  double pis2s2Simple(double p, double q) const;
  /// Calculates transverse part of A^0 self-energy: for p=external momentum,
  /// Q=renormalisation scale
  virtual double piAASimple(double p, double Q) const;
  /// Does the calculation of one-loop pieces of \f$ t_1 / v_1 \f$ 
  virtual double doCalcTadpole1oneLoopSimple(double mt, double sinthDRbar);
  /// Does the calculation of one-loop pieces of \f$ t_2 / v_2 \f$ 
  virtual double doCalcTadpole2oneLoopSimple(double mt, double sinthDRbar);
  /// Calculates and sets the one-loop pieces of \f$ t_1 / v_1 \f$: sets both
  /// 1-loop and total pieces equal to the one-loop piece
  virtual void calcTadpole1Ms1loopSimple(double mt, double sinthDRbar);
  /// Calculates then sets the one-loop pieces of \f$ t_2 / v_2 \f$: sets both
  /// 1-loop and total pieces equal to the one-loop piece
  virtual void calcTadpole2Ms1loopSimple(double mt, double sinthDRbar);
  bool higgsSimple(int accuracy, double piwwt, double pizzt);
};
...
// DH:: boundary conditions in which first and second generation A terms are zero.
