...
#ifndef lowE6SSM_SPECTRUM_GENERATOR_H
...
#include "lowE6SSM_two_scale_initial_guesser.hpp"
...
template <class T>
class lowE6SSM_spectrum_generator {
public:
   ...
   void run(const QedQcd& oneset, const lowE6SSM_input_parameters& input);
   void write_running_couplings(const std::string& filename = "lowE6SSM_rge_running.dat") const;
   void write_spectrum(const std::string& filename = "lowE6SSM_spectrum.dat") const;
 ...
 */
template <class T>
void lowE6SSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   lowE6SSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   lowE6SSM_parameter_getter parameter_getter;
 ...
 */
template <class T>
void lowE6SSM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
   lowE6SSM_spectrum_plotter plotter;
