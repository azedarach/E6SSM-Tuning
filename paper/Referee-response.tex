...
For parts of the parameter space scanned the referee is correct to point
out that there is a danger of color or charge breaking minima, and that
the range of values for $\mu$ scanned over includes values that would be in
conflict with experimental limits.  In our analysis we excluded points
that may lead to such minima by removing those which have tachyons in the
spectrum, and for the MSSM we have additionally checked that included points
satisfy known conditions for the dangerous minima to be avoided.  This however
was not commented on in our submission.  Regarding the LEP2 data and chargino
mass bounds, in Figures 2 and 4 the chargino mass limit was not imposed.
Instead the impact of the chargino bounds is explored in Figure 2.  As can
be inferred from this figure, and we confirmed that this is the case in
detail for all of the models considered, excluding low mass charginos does not
change the lower bound on fine tuning that is obtained.  Nevertheless the
referee is correct in pointing out that it is more consistent to present only
those points that are allowed by the experimental limits, as we have already
done for other limits on sparticle masses.

To address this point, we have updated Figures 2 and 4 to exclude those points
that would conflict with the LEP2 data.  As noted above, this does not alter
the results at all.  In addition, a short discussion has been added on page 17,
prior to Table II, in which the constraints that are applied to the analysis
are clarified.  In particular, we explicitly state that points violating the
LEP chargino limits are now excluded from the analysis.  This is accompanied
by a footnote pointing out that for the region of parameter space studied
whether or not this constraint is applied does not in fact impact the results.
We also explain that in order to exclude points that may have color and charge
breaking minima we only allow points that do not lead to tachyonic sparticle
masses and, in the MSSM, that satisfy the traditional conditions in the
literature for color or charge breaking minima to be avoided.
