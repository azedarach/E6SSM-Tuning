...
However, it is well known in the MSSM that radiative corrections can significantly change (indeed, reduce) the fine tuning \cite{Cassel:2010px}. It is therefore important when studying the fine tuning to include loop corrections to the effective potential in the fine tuning measure. To do so it is most convenient to work in terms of the EWSB conditions Eq. (\ref{eq:E6EWSBConditions}), rather than Eq. (\ref{eq:E6MZequation}). The general procedure is as follows \cite{Ellwanger:2011mu}. For a model in which $m$ fields develop real VEVs (e.g. $m=2$ in the MSSM, $m=3$ in the NMSSM and in the $E_6$ models considered), we require that the $m$ minimization conditions
...
As discussed in the introduction many recent papers have focussed on
light stops as allowing natural SUSY and there has been a lot of
theoretical effort to find SUSY models with a $125$ GeV Higgs boson
while still having light stops and experimental efforts to search for
light stops.  This is entirely appropriate since there are many good
reasons to expect the soft masses to be set at high energies.  However
that is not the only possibility and the fine tuning problem depends
strongly the RG evolution from the GUT scale, as the soft Higgs masses
that appear in the EWSB conditions pick up contributions from the soft
squark masses.
...
% DH:: I'm not sure if this is worth including? I've put a disclaimer in the
% text, as well.
\begin{figure}[h]
\begin{center}
\resizebox{!}{10cm}{%
...
\end{center}
\end{figure}
To illustrate this, in Fig.~\ref{Fig:mssmstoptuning} we show the
variation in fine tuning when we scan over the stop masses and mixing,
with $500 \textrm{ GeV} \leq m_{Q_3},m_{u_3}\leq 10\textrm{ TeV}$ and
$-3810\textrm{ GeV}\leq A_t\leq -20\textrm{ GeV}$, for $M_X=20$ TeV,
$50$ TeV and $100$ TeV.  The remaining parameters we fix such that at
$M_{SUSY}$ they have the values $\mu=-97.5$ GeV, $B=-84.8$ GeV,
$M_1=92.1$ GeV, $M_2=95.9$ GeV, $M_3=352$ GeV, $A_b=-117.9$ GeV,
$A_\tau=-7.8$ GeV, $m_{L_i}=400$ GeV, $m_{e_i}=204$ GeV,
$m_{Q_{1,2}}=438$ GeV, $m_{u_{1,2}}436$ GeV and $m_{d_i}=438$ GeV
($i=1,2,3$). Although we should stress that making this choice will
lead to a spectrum which is in conflict with the LHC limits, doing so
ensures that fine tuning due to the other parameters is small, so that
we avoid washing out the fine tuning impact of the stops when the
tuning is small\footnote{For models in which the spectrum is heavier,
  when the stop masses are small the fine tuning reaches a lower bound
  imposed by other heavier parameters.} as can be the case when the
stop masses are less than 1 TeV.  Note that the Higgs mass is also
allowed to vary in this scan, as shown in
Fig.~\ref{Fig:mssmhiggstuning}. This reveals the tuning problem which
people have been worrying about since the discovery of the 125 GeV
Higgs boson as we see that raising the stop masses is also pushing up
the Higgs mass, meaning that heavier Higgs masses require more fine
tuning.  However for a low value of the UV scale this tuning is not so
severe unless the stops are very heavy, and a 125 geV Higgs can be
obtained without much tuning in this unrealistic case where we have
minimised other sources of tuning.  On the other hand the tuning
becomes more severe as we increase the cuttoff such that for $M_X =
10^{16}$ GeV a lightest stop masses of $1-3$ TeV can result in a fine
tuning of $\approx 200 - 1000$ as shown in
Fig.~\ref{Fig:mssmstoptuning} and the minimum tuning we see consistent
...
of $M_2$ to introduce more variation in the chargino masses, which can
also be importnat due to the link between $\mu$ and the fine tuning.
 ...
 assume their associated soft trilinears vanish [what about
   $A_\tau$?]. The $U(1)$ gaugino soft mass $M_1$ was fixed to
...
\end{tabular}
\end{ruledtabular}
\caption{The parameters scanned over and the ranges of values used in the MSSM and the $E_6SSM$ models.}
...
$Z^\prime$ boson.  So in the case of a very low cutoff much of the
tuning required to get a 125 GeV Higgs has gone away.  However the
tuning from the $Z^\prime$ mass appears already at tree level and is
unavoidable regardless of the cutoff scale chosen, so in the E$_6$SSM
current limits on the mass of the $Z^\prime$ boson imply that the
E$_6$SSM fine tuning must be at least 100 regardless of assumptions
about the way SUSY breaking happens.  Additionally if run II of the
LHC pushes the limit on the $Z^\prime$ mass to be above $4.5$ TeV then
the fine tuning in the model will be greater than at least 350.
...
tuning reqyired by the $125$ GeV Higgs measurement which concerns
people in the MSSM.
...
Unfortunately the chargino limits from the LHC are not general enough
so as to provide an absolute constraint on the model [Add: XSdetails on
  limits and what they depend on].

Nonetheless the MSSM the impact of chargino limit is shown in
Fig.~\ref{}.  There we see that if the the full parameter space with
$m_{\chi^\pm} < 700$ GeV was excluded the impact would be to make the
tuning in the MSSM with a $20$ TeV cuttoff slightly worse than that of
the E$_6$SSM with the same cuttoff and a $Z^\prime$ mass just larger
than current limits.  would be similar to that can also be attcked
more directly by looking at chargino mass limits as this rasies $\mu$
directly.  In the E$_6$SSM, while raising the chargino limit can have
the same impact in princuiple, due to current limits on the $Z^\prime$
mass already imposing a significant degree of tuning, chargino masses
do not make much of a noticable change.

[show plots in MSSM and E6SSM if it gets clened up, though maybe we
  can just leave it out if it only shows that there is no big impact?]
...
% - plots for different U(1)' charge 

% DH:: for reference:
%      U(1)_I \theta = \pi+\arctan\sqrt{3/5}
%      U(1)_\psi \theta = \pi / 2
%      U(1)_\eta \theta = -\arctan\sqrt{5/3}
...
case of the $U(1)_\eta$.  This allows a dramatic reduction in the fine
...
mass in the same way as happens in the other models, explaining why
...
of the E$_6$SSM, with the cuttoff at the GUT scale, is considered and
...
picture of a natural SUSY model emerged from theoretcal reasoning,
...
constrained models, and then the LHC meaured the Higgs to be around
$125$ GeV.
