...
\ee where
\begin{eqnarray}
W_0 &=& \lambda_{ijk} \hat{S}_i  \hat{H}^d_{j} \cdot\hat{H}^u_{k} + \kappa_{ijk} \hat{S}_i \hat{D}_j \hat{\bar{D}}_k + h^N_{ijk} \hat{N}^c_i  \hat{H}^u_{j} \cdot \hat{L}_k \nonumber \\
& & + h^U_{ijk} \hat{u}^c_i  \hat{H}^u_{j} \cdot \hat{Q}_k + h^D_{ijk} \hat{d}^c_i  \hat{Q}_k\cdot\hat{H}^d_{j}  + h^E_{ijk} \hat{e}^c_i  \hat{L}_k\cdot\hat{H}^d_{j} , \\
...
%%   \begin{split}
    ...
    \cdot \SuperField{H}_{2i}  + \kappa_i \SuperField{S} \SuperField{D}_i
...
H_2|^2,\label{eq:E6VFterms}\\ 
V_D&=\frac{\bar{g}^2}{8}\left (
|H_2|^2-|H_1|^2\right )^2+\frac{g_2^2}{2}|H_1^\dagger
H_2|^2+\frac{g_1'^2}{2}(Q_1|H_1|^2+Q_2|H_2|^2+Q_S|S|^2)^2,\label{eq:E6VDterms}\\ 
V_{\textrm{soft}}&=m_S^2|S|^2+m_{H_d}^2|H_1|^2+m_{H_u}^2|H_2|^2+\Big
[\lambda A_\lambda SH_1\cdot H_2+\textrm{h.c.}\Big
].\label{eq:E6Vsoft}
\end{align}
...
$H_1$, $H_2$ and $S$, respectively. In the case
...
(v_2^2-v_1^2)v_1+D_{H_1}v_1+\frac{\partial \Delta V}{\partial v_1}=0,\label{eq:E6EWSBcondition1} \\
&f_2=m_{H_u}^2v_2+\frac{\lambda^2}{2}(v_1^2+s^2)v_2-\frac{\lambda A_\lambda}{\sqrt{2}}sv_1+\frac{\bar{g}^2}{8}
(v_2^2-v_1^2)v_2+D_{H_2}v_2+\frac{\partial \Delta V}{\partial v_2}=0,\label{eq:E6EWSBcondition2} \\
...
\begin{align}
  m_{\tilde{t}_{1,2}}^2&=\frac{1}{2}\bigg \{ m_{Q3}^2+m_{u3}^2+\frac{1}{2}M_Z^2\cos 2\beta+D_Q+D_u+2m_t^2\nonumber\\
&\quad{}\mp\sqrt{\left [ m_{Q3}^2+m_{u3}^2+\left ( \frac{1}{2}-\frac{4}{3}\sin^2\theta_W\right )M_Z^2\cos 2\beta + D_Q-D_u\right ]^2+4m_t^2X_t^2}\bigg \},
\end{align}
where $m_t^2=y_t^2v_2^2/2$ and $X_t=A_t-\frac{\lambda s v_1}{\sqrt{2}v_2}$. By definition we take $m_{\tilde{t}_1}$ to correspond to the lighter of the two states.\\
...
\caption{The parameters scanned over and the ranges of values used in
  the MSSM and the E$_6SSM$ models.}
...
There is another limit which play a similar role.  Chargino limits
...
using the $d$ coeffioceint one can estimate that $Z^\prime$ limits
...
GeV \cite{Barate:2003sz,Schael:2006cr}, which already introduced
...
bound. Recently this problem got much worse since the LHC measured the
Higgs to be around $125$ GeV.
