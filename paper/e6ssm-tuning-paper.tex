...
To further illustrate this point we looked at how the tuning compares for
...
the green curve in the top left panel. In thus case we have chosen a
point with large mixing which reduce the MSSM tuning somewhat.  this
does not eliminate the tuning as one there is still a string
sensitivity to $A_t$, but we did find that increasing $A_t$ lead less
fine tuning overall.

The E$_6$SSM tunings are rather more complicated.  By comparing them
to the MSSM tuning one can see that the scale at which the parameters
are defined is very important for determining which point is more fine
tuned.  In the bottom left panel one can see that the dependence of
the E$_6$SSM tuning on the cutoff is rather complicated, illustrating
that any statement about which model is more tuned depends on the
cutoff scale considered.

For E$_6$SSM BM1 the fine tuning is shown by the blue curve in top
left panel of Fig.~\ref{Fig:BMs-varyMX} and the individual sensitivities
are given in the bottom left panel. The tuning actually reduces
initially as the cutoff is is increased from $20$ TeV.  This occurs
because the tuning is initially dominated by $\Delta_\lambda$ (shown in
solid blue in the bottom left panel) that contains some terms
proportional to $M_Z^{\prime \, 2}$, which currently dominate the
tuning.  However as the cutoff scale is increased contributions from
the soft masses become more important and these actually start to
cancel the large contribution to $\Delta_\lambda$ coming from
$M_{Z^\prime}$ until $\Delta_\lambda$ passes through zero.  However at
the same time these large soft masses also cause other sensitivities
to grow, leading the overall tuning to start rising with $M_X$
once. Nonetheless due to the lower stop masses and lower mixing [I'm
  guessing slightly: Dylan I need to be able to refer to numbers
  here!]  compared to both of the other points until $M_X \approx
10^{11}$ GeV.  After this the tuning from $M_3$ starts to dominate in
this point.  Although at $M_{SUSY}$ this point as the same $M_3$ value
as the MSSM point, in the E$_6$SSM $M_3$ will be larger at $M_X$ due
to the altered RGE running from exotic matter\footnote{This altered
  ...
  $U(1)$ anomaly free rather than being a direct consequence of the
  extra $U(1)$. }.  At the same time larger $M_3$ at higher scales
reduces the soft squark masses at $M_X$.  Therefore the stop mass
contributions are ameliorated, compared to the MSSM, both by allowing
lighter stops at $M_{SUSY}$ and by the modified RGE running.
Nonetheless the stops still have an impact, pushing up the sensitivity
to $m_{H_u}^2$\footnote{$m_{H_u}^2(M_{SUSY})$ receives a positive
  contribution to it's mass from $m_{H_u}^2(M_X)$ and a negative
  contribution from $m_{Q_3}^2$. If $m_{H_u}^2(M_{SUSY})$ is held
  fixed so that then as the cutoff increases the values of these
  softmasses at $M_X$ will be larger and there we be a bigger
  cancellation between them, making the sensitivity of $M_Z$ to both of
  these parameters larger.}  showing the most commonly discussed
aspect of the little hierarchy problem.
...
$\Delta_{A_t}$ as we did in the MSSM benchmark.  [Last thing that needs to be explained - why do $\Delta_{M_3}$ and $\Delta_{m_{H_u}^2}$ start to decrease at $M_X$.  I'm guessing something funny to do with their values and also  $m_{Q}^2$ and $M_3$ at the highscales.  but i need to see the numbers to know.]


\begin{figure}
\begin{center}
\resizebox{!}{5.5cm}{%
\includegraphics{e6ssm_mssm_cutoff_scan_3_points_tuning_plot.png}} 
\resizebox{!}{5.5cm}{%
...
\resizebox{!}{5.5cm}{%
\includegraphics{e6ssm_light_zprime_cutoff_scan_tb10_fixed_at_ms_mssm_mixing_small_M3_all_tuning_data.png}}%%
\caption{{\bf Top left panel:} Scatter plot of fine tuning vs high scale $M_X$ for the three benchmark points given in (?).  {\bf Top right panel:}  Individual sensitivites of MSSM BM plotted against the high scale $M_X$ which give an overall tuning shown by the green line in top left panel.  {\bf Bottom left panel:} Individual sensitivites of E6SSM BM1 plotted against the high scale $M_X$ which give an overall tuning shown by the blue line in top left panel.  {\bf Bottom right panel:}Individual sensitivites of E6SSM BM2 plotted against the high scale $M_X$ which give an overall tuning shown by the red line in top left panel.    }
