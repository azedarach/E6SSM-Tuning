...
To further illustrate this point we looked at how the tuning compares
for low tuning benchmarks in the MSSM and E6SSM when the highscale
boundary condition, $M_X$ is raised. The results of this are shown in
Fig.~\ref{Fig:BMs-varyMX}. Since the behaviour is a quite complicated
we now discuss these in detail as it provides some insight into the
many differences in the tuning between the two models.
...
fine tuning overall fo the points we looked at.

The E$_6$SSM tunings are rather more complicated as there are more
sources of fine tuning.  By comparing them to the MSSM tuning one can
see that the scale at which the parameters are defined is very
important for determining which point is more fine tuned.  In the
bottom left panel one can see that the dependence of the E$_6$SSM
tuning on the cutoff is rather complicated, illustrating that any
statement about which model is more tuned depends on the cutoff scale
considered.
