...
To further illustrate point we looked at how the tuning compares for
low tuning benchmarks in the MSSM and E6SSM when the highscale
boundary condition, $M_X$ is raised.  
...
Fig.~\ref{Fig:BMs-varyMX} where in the top left panel we see that the
dependence of the E6SSM tuning on the cutoff is rather complicated
leading, such that any statement about which model is more tuned
depends on the cutoff scale considered.
...
sensitivities, in particular $\Delta_{m_{H_u}^2}$ and $\Delta_\lambda$.
