...
models with a $125$ GeV Higgs bosons while still having light stops
...
In Fig. \ref{} we fix blah blah bah [Dylan insert details please] and
show how fine tuning varies as we increase the stop masses. However
thius tuning inly really becomes large for very large stop masses.  On
the other hand if cuttoff is increased then stop masses of severalk
...
the impact of the $Z^\prime$ mass on fine tuning. To do se we cjoose a
cutoff of [insert cutoff] look at the fine tuning for two different
...
$Z^\prime$ masses as we did for the E$_6$SSM.
 % - if room, plots for different U(1)' charge Here we see that the As
can be seen in Fig.~\ref{} the severity of the tunings varies quite a
...
\begin{align}
\beta_\lambda^{(1)}&=,\label{eq:USSMLambdaBetaOneLoop}\\
\beta_\lambda^{(2)}&=.\label{eq:USSMLambdaBetaTwoLoop}
\end{align}
\end{subequations}
