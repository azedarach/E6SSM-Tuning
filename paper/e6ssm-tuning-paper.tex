...
%% These very well motovated supersymmetric models predict a new
%% $Z^\prime$ boson which could be discovered at the LHC.  like the
%% superpartners naturalness of the model requires that the $Z^\prime$
%% boson mass should not be too \far above the TeV scale.  Moreover this
%% fine tuning appears at the tree level, making it less dependent on a
%% SUSY breaking mecahnism which has not been fully determnined.  Here we
%% study this fine tuning for several $U(1)$ supesymmetric extensions of
%% the standard model and compare it to the situation in the MSSM where
%% the most direct tree level fine tuning can be probed through chargino
%% mass limits.  we show that future LHC $Z^\prime$ searches are
%% extremely important for challenging the most natural scenarios in
%% these models.
...
require moderate fine tuning in the E6SSM.  We also show this is
...
is $U()_N$,under which the right handed neutrino remains
...
\cite{}[CITE PLEASE] and then compare the fine tuning calculated in
...
play a significant role in reducing the fine tuning [WE SHOULD DISCUSS
  THIS].  For this reason it is still important to properly treat the
...
therefore not heavily suppressed when the cutoff scale is low. We find
...
$\kappa_{1,2,3}(M_{\mathrm{SUSY}})$ & $-$ & 0.6 & 0.6 \\
$\lambda_{1,2}(M_{\mathrm{SUSY}})$ & $-$ & 0.2 & 0.2 \\
$\mu_{\mathrm{eff}}(M_{\mathrm{SUSY}})$ [GeV] & 689.7 & 1093.3 & -1093.3 \\
$B_{\mathrm{eff}}(M_{\mathrm{SUSY}})$ [GeV] & 345.7 & 3792.7 & -3792.7 \\
$A_t(M_{\mathrm{SUSY}})$ [GeV] & -3335.7 & -1100 & -3335.7 \\
$A_b(M_{\mathrm{SUSY}}), A_\tau (M_{\mathrm{SUSY}})$ [GeV] & 0 & 0 & 0 \\
$m_{Q3}^2(M_{\mathrm{SUSY}})$ [GeV$^2$] & $4.45379310\times 10^6$ & $4.49655\times 10^5$ & $6.5\times 10^5$ \\
$m_{u3}^2(M_{\mathrm{SUSY}})$ [GeV$^2$] & $4\times 10^6$ & $5.86207\times 10^5$ & $4.5\times 10^5$ \\
$M_1(M_{\mathrm{SUSY}})$ [GeV] & 300 & 300 & 300 \\
$M_2(M_{\mathrm{SUSY}})$ [GeV] & 2000 & 1050 & 2000 \\
$M_3(M_{\mathrm{SUSY}})$ [GeV] & 2000 & 2000 & 1200 \\
$M_1^\prime(M_{\mathrm{SUSY}})$ [GeV] & $-$ & 300 & 300 \\
\hline
$M_{Z^\prime}$ [GeV] & $-$ & 2473.2 & 2473.1 \\
$m_{h_1}$ [GeV] & 124.3 & 125.0 & 124.9 \\
$m_{\tilde{t}_1}$ [GeV] & 1942.1 & 993.8 & 839.6 \\
$m_{\tilde{t}_2}$ [GeV] & 2220.1 & 1174.8 & 1285.2 \\
$m_{\tilde{g}}$ [GeV] & 2259.8 & 2290.0 & 1453.8\\
\hline
$\Delta(M_X=20\textrm{ TeV})$ & 157.3 & 165.3 & 228.5 \\
$\Delta(M_X=10^{16}\textrm{ GeV})$ & 1089.0 & 1722.3 & 917.6
\end{tabular}
\end{ruledtabular}
\caption{Parameters and masses for the MSSM point and the E$_6$SSM benchmark points 
BM1 and BM2 for which the fine tuning as a function of cutoff scale is studied. For the
E$_6$SSM benchmarks we define $\mu_{\mathrm{eff}}\equiv \lambda s/\sqrt{2}$ and
$B_{\mathrm{eff}}=A_\lambda$. All other (diagonal) soft scalar squared masses not shown above are set to $2.5\times 10^7$ GeV$^2$, and all soft trilinears are set to zero. In the E$_6$SSM, we additionally set $\mu^\prime=5000$ GeV and $B\mu^\prime=5000$ GeV$^2$.}
...
\begin{center}
\resizebox{!}{5.5cm}{%
\includegraphics{e6ssm_mssm_cutoff_scan_all_benchmark_points_tuning_vs_cutoff.png}} 
\resizebox{!}{5.5cm}{%
...
\caption{{\it Top left panel:} Scatter plot of fine tuning vs high scale $M_X$ for the three benchmark points given in Table \ref{tab:benchmarks}.  {\it Top right panel:}  Individual sensitivities of the MSSM benchmark plotted against the high scale $M_X$ which give an overall tuning shown by the green line in top left panel.  {\it Bottom left panel:} Individual sensitivities of E6SSM BM1 plotted against the high scale $M_X$ which give an overall tuning shown by the blue line in top left panel.  {\it Bottom right panel:} Individual sensitivities of E6SSM BM2 plotted against the high scale $M_X$ which give an overall tuning shown by the red line in top left panel.    }
...
10^{11}$ GeV.  Eventually the $\Delta_{M_3}$ sensitivity leads to
...
level tuning from $M_{Z^\prime}$ dominates.  However the reduction in
$M_3$ means that $\Delta_{M_3}$ is substantially lower and only
becomes the dominant tuning very close to the GUT scale, giving a
tuning at $10^{16}$ GeV of $\approx 900$, which is below that of the
other two benchmark points.  Since large mixing was also used to
reduce allow lighter stops, we see initially a large contribution from
$\Delta_{A_t}$ as we did in the MSSM benchmark.  [Last thing that
  needs to be explained - why do $\Delta_{M_3}$ and
  $\Delta_{m_{H_u}^2}$ start to decrease at large $M_X$.  I'm guessing
  something funny to do with their values and also $m_{Q}^2$ and $M_3$
  at the high scales.  but i need to see the numbers to know.]
