 ...
 */
double genericE6SSM_susy_parameters::calc_beta_g3_one_loop(const Susy_traces& susy_traces) const
{
 ...
 */
double genericE6SSM_susy_parameters::calc_beta_g3_two_loop(const Susy_traces& susy_traces) const
{
   ...
   beta_g3 = Power(g3,3)*twoLoop*(-2*traceKappaAdjKappa - 4*traceYdAdjYd
      - 4*traceYuAdjYu + 3*Sqr(g1) + 9*Sqr(g2) + 48*Sqr(g3) + 6*Sqr(gN)*Sqr(Qdp
      ) + 6*Sqr(gN)*Sqr(QDxbarp) + 6*Sqr(gN)*Sqr(QDxp) + 12*Sqr(gN)*Sqr(QQp) +
      6*Sqr(gN)*Sqr(Qup));
