...
#ifndef genericE6SSM_SPECTRUM_GENERATOR_H
#define genericE6SSM_SPECTRUM_GENERATOR_H
...
#include "genericE6SSM_two_scale_convergence_tester.hpp"
#include "genericE6SSM_two_scale_initial_guesser.hpp"
#include "genericE6SSM_utilities.hpp"
...
template <class T>
class genericE6SSM_spectrum_generator {
public:
   genericE6SSM_spectrum_generator()
      : solver(), model()
   ...
   const genericE6SSM<T>& get_model() const { return model; }
   const Problems<genericE6SSM_info::NUMBER_OF_PARTICLES>& get_problems() const {
   ...
   void run(const QedQcd& oneset, const genericE6SSM_input_parameters& input);
   void write_running_couplings(const std::string& filename = "genericE6SSM_rge_running.dat") const;
   void write_spectrum(const std::string& filename = "genericE6SSM_spectrum.dat") const;

private:
   RGFlow<T> solver;
   genericE6SSM<T> model;
   genericE6SSM_high_scale_constraint<T> high_scale_constraint;
   genericE6SSM_susy_scale_constraint<T> susy_scale_constraint;
   genericE6SSM_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void genericE6SSM_spectrum_generator<T>::run(const QedQcd& oneset,
                                const genericE6SSM_input_parameters& input)
{
   ...
   genericE6SSM_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   genericE6SSM_initial_guesser<T> initial_guesser(&model, input, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void genericE6SSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   genericE6SSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   genericE6SSM_parameter_getter parameter_getter;
   Coupling_monitor<genericE6SSM<T>, genericE6SSM_parameter_getter>
 ...
 */
template <class T>
void genericE6SSM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
   genericE6SSM_spectrum_plotter plotter;
