...
#ifndef genericE6SSM_UTILITIES_H
#define genericE6SSM_UTILITIES_H
...
class genericE6SSM_parameter_getter {
public:
   Eigen::ArrayXd get_parameters(const genericE6SSM<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const genericE6SSM<Two_scale>&) const {
      using namespace genericE6SSM_info;
      return std::vector<std::string>(parameter_names,
                                      parameter_names + NUMBER_OF_PARAMETERS);
   }
};

class genericE6SSM_spectrum_plotter {
public:
   genericE6SSM_spectrum_plotter();
   ~genericE6SSM_spectrum_plotter() {}

   template <class T>
   void extract_spectrum(const genericE6SSM<T>&);
...
template <class T>
void genericE6SSM_spectrum_plotter::extract_spectrum(const genericE6SSM<T>& model)
{
   spectrum.clear();
...
template <class Scalar, int M, int N>
std::valarray<double> genericE6SSM_spectrum_plotter::to_valarray(const Eigen::Array<Scalar, M, N>& v)
{
